...
// 构造方法
	...
	History.setWrapStyleWord(true);  // 激活断行不断字功能        
	...
	for (int i = 0; i < KEYS.length; i++)
	{
	    ...
	    keys[i].setBounds(x, y, 60, 40); //7按钮的位置 20，150
	    if (x < 215) {
		x += 65;                        //8按钮的位置 20+60+5=85 ，150
	    } else {
		x = 20;                         //4按钮的位置 20，150+40+5=195
		y += 45;
	    }
	    this.add(keys[i]);
	}
	...
	this.setResizable(false);        //不能调整窗体大小
...
// 事件处理
	...
	if(label=="=")//
	{
	    resultText.setText(this.b);  //把文本框置空
